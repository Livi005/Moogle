            ...
            for (int i = 0; i < querys.Count; i++)
            {
                ...
                for (int j = 0; j < textos.Count; j++)
                {
                    if (!b[j])
                    {
                        ...
                        Tf[i, j] = frecuencia_tf > 0 ? 1 + (float)Math.Log10(frecuencia_tf) : 0;

                        frecuencia_idf = frecuencia_tf > 0 ? frecuencia_idf + 1 : frecuencia_idf;
                    }
                }

                //Calculo del IDF. Pongo mas 1 para evitar que se divida entre cero.
                IDF[i] = (float)Math.Log10((textos.Count / (frecuencia_idf + 1)));
            }
